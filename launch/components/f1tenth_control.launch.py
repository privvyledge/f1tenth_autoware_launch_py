...
def launch_setup(context, *args, **kwargs):
    ...
    external_cmd_converter_loader = IncludeLaunchDescription(
        ...
        launch_arguments=[
            ...
            ('target_container', '/control/control_container'),
            ('ref_vel_gain', '0.0'),
            ('csv_path_accel_map', PathJoinSubstitution([LaunchConfiguration('csv_path_map'), 'accel_map.csv'])),
            ('csv_path_brake_map', PathJoinSubstitution([LaunchConfiguration('csv_path_map'), 'brake_map.csv']))
        ],
    )
